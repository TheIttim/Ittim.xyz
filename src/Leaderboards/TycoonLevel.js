...
export default function BalanceLB() {
  function usePromise() {
    ...
    useEffect(() => {
      ...
      fetch(`${config.API_URL}${config.ENDPOINTS.LEADERBOARDS}/tycoonlevel`)
        .then(async (res) => {
          if (isMounted) {
            ...
            for (let i = 0; i < data.length; i++) {
              ...
              rows.push(
                <tr key={i} data-id={row.id}>
                  <th scope="row">{i + 1}</th>
                  <th scope="row">{row.tycoon.name}</th>
                  <td>
                    <div className="row">
                  ...
                  <td>{row.tycoon.level.toLocaleString()}</td>
                </tr>
              );
            }
        ...
        <table class="table table-striped table-hover">
          <thead>
            <tr>
              ...
              <th scope="col">Tycoon Name</th>
              <th scope="col">Tycoon Owner</th>
              <th scope="col">Tycoon Level</th>
            </tr>
          </thead>
          <tbody>{data}</tbody>
        </table>
